...
extension Bundle {
    ...
    func reciterIds() -> [Int] {
        guard let url = url(forResource: "reciters_manifest", withExtension: "json") else {
            AppLogger.shared.warn("reciters_manifest.json not found in \(bundleIdentifier ?? "bundle")", category: .network)
            return []
        }
        struct Entry: Codable { let id: Int }
        guard let data = try? Data(contentsOf: url),
              let entries = try? JSONDecoder().decode([Entry].self, from: data) else {
            AppLogger.shared.warn("Failed to decode reciters_manifest.json", category: .network)
            return []
        }
